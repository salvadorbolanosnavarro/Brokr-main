            ...
            while page <= 80:  # up to 4000 properties
                ...
                for p in props:
                    ...
                    if not loc or normalize(ciudad) not in normalize(loc):
                        continue
                    # Status: published, unpublished, suspended, sold, reserved
                    status = p.get("status", "").lower()
                    ALLOWED = ("published","publicada","no publicada","unpublished",
                               "suspendida","suspended","vendida","sold",
                               "apartada","reserved","activa","active","")
                    ...
                    if updated:
                        try:
                            ...
                            if year < 2025 or (year == 2025 and month < 1):
                                continue
                        except:
                            pass
                    else:
    ...
    async with httpx.AsyncClient(timeout=20) as client:
        while len(comparables) < 50 and page <= 20:
            ...
            for p in props:
                ...
                # Colonia match — strict normalized comparison
                def norm(s):
                    for a,b in [('á','a'),('é','e'),('í','i'),('ó','o'),('ú','u'),('ñ','n')]:
                        s = s.lower().replace(a,b)
                    return re.sub(r'[^a-z0-9 ]', '', s).strip()

                if colonia:  # only filter if colonia provided
                    col_norm = norm(colonia)
                    loc_norm = norm(loc)
                    # Must match: colonia name in location string
                    if col_norm not in loc_norm:
                        continue
                ...
                # ── STATUS FILTER: published, unpublished, suspended, sold, reserved ──
                status = p.get("status", "").lower()
                ALLOWED = ("published","publicada","no publicada","unpublished",
                           "suspendida","suspended","vendida","sold",
                           "apartada","reserved","activa","active","")
                ...
                # ── DATE FILTER: January 2025 onwards ──
...
async def calcular_avm(req: AVMRequest):
    ...
    if len(comparables_raw) < 3:
        nivel = 2
        # Try city-wide search with NO colonia filter
        # But we'll apply strict price/m2 filtering after
        comparables_raw = await get_comparables_eb(
            "", req.ciudad, req.tipo, req.operacion
        )
        nivel_msg = (f"Pocos comparables en {req.colonia} con datos 2025-2026. "
                     f"Se amplió a {req.ciudad} — resultados filtrados por precio/m².")
    ...
    return {
        ...
        "nota": ("Valores calculados con base en propiedades publicadas en la bolsa "
                 "EasyBroker (ene 2025–mar 2026), con ajustes hedónicos por m², recámaras, "
