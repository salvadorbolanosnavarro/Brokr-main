    ...
    async with httpx.AsyncClient(timeout=60) as client:
        while len(comparables) < 50 and page <= 160:
            ...
            for p in props:
                ...
                # ── 4. DATE: extract publication year for price update ──
                published_at = p.get("published_at", "") or p.get("updated_at", "") or p.get("created_at", "")
                pub_year = 2025  # default if no date
                if published_at:
                    try:
