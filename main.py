...
def build_ficha_html(p: dict, images_b64: dict) -> str:
    """Build complete self-contained HTML for PDF generation."""
    ...
    precio    = f"${monto:,.0f} {moneda}" if monto else "—"
    ...
    def fmt_m2(n):
        if not n: return "—"
        return f"{n:,.2f} m²".replace(".00", "")
    
    # Specs
    specs = []
    if rec:     specs.append(("🛏", rec, "Recámaras"))
    if ban:     specs.append(("🚿", ban, "Baños"))
    if m2c:     specs.append(("📐", m2c, "m² Const."))
    if m2t:     specs.append(("🌿", m2t, "m² Terreno"))
    if parking and len(specs) < 4: specs.append(("🚗", parking, "Estacion."))
    
    specs_html = ""
    if specs:
        items = "".join(f"""<div class="spec-item"><div class="spec-ico">{s[0]}</div>
            <div class="spec-val">{s[1]}</div><div class="spec-lbl">{s[2]}</div></div>""" for s in specs[:4])
        specs_html = f'<div class="cover-specs">{items}</div>'
    ...
    hero_html = f'<img class="cover-hero" src="{hero_src}" alt="portada"/>' if hero_src else '<div class="cover-hero-placeholder"></div>'
    
    # Footer
    def footer():
        return f'''<div class="ficha-footer">
            <img src="{LOGO}" class="ft-logo" alt="Brokr"/>
            <div class="ft-id">{id_prop}</div>
        </div>'''
    
    # Gallery pages (6 per page, starting from foto index 1)
    gallery_fotos = foto_urls[1:]
    gallery_pages = ""
    for i in range(0, len(gallery_fotos), 6):
        batch = gallery_fotos[i:i+6]
        imgs = "".join(
            f'<img src="{images_b64.get(u, u)}" alt="foto"/>' if u else '<div class="photo-empty"></div>'
            for u in (batch + [""] * (6 - len(batch)))
        )
        gallery_pages += f'''
        <div class="ficha-page">
            <div class="gallery-header"><h2>Galería fotográfica</h2></div>
    ...
    if tipo_op:   rows.append(("Operación", tipo_op))
    if precio:    rows.append(("Precio", precio))
    if rec:       rows.append(("Recámaras", rec))
    if ban:       rows.append(("Baños completos", ban))
    if p.get("half_bathrooms"): rows.append(("Medios baños", p["half_bathrooms"]))
    if m2c:       rows.append(("Superficie construida", fmt_m2(m2c)))
    if m2t:       rows.append(("Superficie de terreno", fmt_m2(m2t)))
    if parking:   rows.append(("Estacionamientos", parking))
    if p.get("floors"): rows.append(("Niveles", p["floors"]))
    if colonia:   rows.append(("Colonia", colonia))
    if ciudad:    rows.append(("Ciudad", ciudad))
    if id_prop:   rows.append(("Clave", id_prop))
    
    rows_html = "".join(f'<tr><td class="char-lbl">{k}</td><td class="char-val">{v}</td></tr>' for k, v in rows)
    
    amen_html = ""
    if amenids:
        items = "".join(f'<div class="amen-item">{a.get("name") or a}</div>' for a in amenids)
        amen_html = f'<div class="amen-section"><div class="amen-ttl">Amenidades</div><div class="amen-grid">{items}</div></div>'
...
.cover-hero-placeholder{{width:100%;height:110mm;background:#1a2744;flex-shrink:0}}
.cover-info{{padding:16px 24px 12px;border-bottom:1px solid #e8ecf2}}
.cover-badge{{display:inline-block;background:#c9a84c;color:white;font-size:9px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;padding:4px 10px;border-radius:20px;margin-bottom:8px}}
.cover-precio{{font-family:'Playfair Display',serif;font-size:28px;font-weight:700;color:#0f1829;line-height:1;margin-bottom:5px}}
.cover-titulo{{font-size:13px;font-weight:600;color:#0f1829;margin-bottom:3px}}
.cover-ubicacion{{font-size:11px;color:#6b7a99}}
.cover-specs{{display:grid;grid-template-columns:repeat(4,1fr);border-bottom:1px solid #e8ecf2}}
.spec-item{{padding:10px 16px;text-align:center;border-right:1px solid #e8ecf2}}
.spec-item:last-child{{border-right:none}}
.spec-ico{{font-size:16px;margin-bottom:2px}}
.spec-val{{font-size:15px;font-weight:700;color:#0f1829}}
.spec-lbl{{font-size:8px;text-transform:uppercase;letter-spacing:.5px;color:#6b7a99;margin-top:1px}}
.cover-desc-wrap{{padding:14px 24px;flex:1;overflow:hidden}}
.cover-desc-ttl{{font-family:'Playfair Display',serif;font-size:12px;font-weight:600;color:#0f1829;margin-bottom:6px;padding-bottom:5px;border-bottom:2px solid #2a9db5;display:inline-block}}
.cover-desc{{font-size:11px;color:#333;line-height:1.6}}
.gallery-header{{padding:14px 24px 10px;border-bottom:1px solid #e8ecf2;flex-shrink:0}}
.gallery-header h2{{font-family:'Playfair Display',serif;font-size:15px;font-weight:600;color:#0f1829}}
.photo-grid{{display:grid;grid-template-columns:1fr 1fr;grid-template-rows:82mm 82mm 82mm;gap:2px;padding:2px;flex-shrink:0;height:246mm;overflow:hidden}}
.photo-grid img{{width:100%;height:100%;object-fit:cover;display:block}}
.photo-empty{{background:#f5f6f8}}
.chars-header{{padding:16px 24px 12px;border-bottom:1px solid #e8ecf2;flex-shrink:0}}
.chars-header h2{{font-family:'Playfair Display',serif;font-size:15px;font-weight:600;color:#0f1829}}
.chars-body{{padding:14px 24px;flex:1;overflow:hidden}}
.char-table{{width:100%;border-collapse:collapse}}
.char-table tr{{border-bottom:1px solid #e8ecf2}}
.char-table tr:nth-child(even) td{{background:#f5f6f8}}
.char-lbl{{padding:7px 12px;font-size:11px;color:#6b7a99;width:45%}}
.char-val{{padding:7px 12px;font-size:11px;color:#0f1829;font-weight:600;text-align:right}}
.amen-section{{margin-top:14px}}
.amen-ttl{{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:#2a9db5;margin-bottom:6px}}
.amen-grid{{display:flex;flex-wrap:wrap;gap:5px}}
.amen-item{{font-size:10px;padding:4px 9px;background:#f5f6f8;border-radius:20px;color:#0f1829;border:1px solid #e8ecf2}}
.ficha-footer{{width:100%;height:40px;background:#0f1829;display:flex;align-items:center;justify-content:space-between;padding:0 20px;flex-shrink:0;margin-top:auto}}
.ft-logo{{height:15px;filter:brightness(10)}}
.ft-id{{font-size:9px;color:rgba(255,255,255,.4);letter-spacing:.5px}}
...
<div class="ficha-page">
    ...
    <div class="cover-info">
        <div class="cover-badge">{tipo_op}</div>
        <div class="cover-precio">{precio}</div>
        <div class="cover-titulo">{titulo}</div>
        <div class="cover-ubicacion">{ubicacion}</div>
    </div>
    {specs_html}
    {"<div class='cover-desc-wrap'><div class='cover-desc-ttl'>Descripción</div><div class='cover-desc'>" + desc + "</div></div>" if desc else "<div style='flex:1'></div>"}
    {footer()}
</div>
...
<div class="ficha-page">
    <div class="chars-header"><h2>Características del inmueble</h2></div>
    <div class="chars-body">
        <table class="char-table"><tbody>{rows_html}</tbody></table>
        {amen_html}
    </div>
    {footer()}
</div>
