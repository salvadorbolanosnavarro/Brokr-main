...
class MainAuthRegressionTests(unittest.TestCase):
    def test_main_uses_core_user_id_auth(self):
        ...
        # directly: one remains in main (Empresas) and checkout owns the other.
        self.assertEqual(source.count('/auth/v1/user'), 1)
        self.assertEqual(checkout.count('/auth/v1/user'), 1)
        self.assertEqual(source.count('/auth/v1/user') + checkout.count('/auth/v1/user'), 2)
