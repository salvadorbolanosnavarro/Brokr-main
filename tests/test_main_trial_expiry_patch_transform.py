"""Dry-run guard for the best-effort trial-expiry PATCH Core migration."""
...
MAIN = ROOT / "main.py"
SCRIPT = ROOT / "scripts" / "refactor_main_trial_expiry_patch_core.py"

spec = importlib.util.spec_from_file_location("trial_expiry_patch_transform", SCRIPT)
transform = importlib.util.module_from_spec(spec)
assert spec and spec.loader
spec.loader.exec_module(transform)


class MainTrialExpiryPatchTransformTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_best_effort_semantics_are_preserved(self):
        ...
        self.assertIn('{"id": f"eq.{sub_id}"}', new)
        self.assertIn('"status": "expired"', new)
        self.assertIn('datetime.utcnow().isoformat()', new)
        self.assertIn('timeout=8', new)
