"""Dry-run guard for CSV-import existing-contact PATCH Core routing."""
...
MAIN = ROOT / "main.py"
SCRIPT = ROOT / "scripts" / "refactor_main_csv_contact_patch_core.py"

spec = importlib.util.spec_from_file_location("csv_contact_patch_transform", SCRIPT)
transform = importlib.util.module_from_spec(spec)
assert spec and spec.loader
spec.loader.exec_module(transform)


class MainCsvContactPatchTransformTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_status_filter_cache_and_transport_contract_are_preserved(self):
        new = transform.NEW
        self.assertIn('await patch_rows(', new)
        self.assertIn('"contactos"', new)
        self.assertIn('{"id": f"eq.{contacto_id}"}', new)
        ...
        self.assertIn('existente.update(patch)', new)
