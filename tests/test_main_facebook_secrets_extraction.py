...
ENCRYPT_ROUTER = ROOT / "routers" / "facebook_encrypt_tokens.py"
...
class FacebookSecretsExtractionTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.encrypt_router = ENCRYPT_ROUTER.read_text(encoding="utf-8")
    ...
    def test_encrypt_tokens_route_preserves_fail_closed_availability_check(self):
        self.assertIn(
            "from core.facebook_secrets import facebook_secret_encryption_available",
            self.encrypt_router,
        )
        self.assertIn("if not facebook_secret_encryption_available():", self.encrypt_router)
        self.assertIn("status_code=503", self.encrypt_router)
        self.assertIn("TOKEN_ENC_KEY", self.encrypt_router)
    ...
    def test_files_compile(self):
        ...
        compile(self.encrypt_router, "routers/facebook_encrypt_tokens.py", "exec")
