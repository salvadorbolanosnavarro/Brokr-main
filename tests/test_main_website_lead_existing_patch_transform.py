"""Dry-run guard for website-lead existing-contact PATCH Core migration."""
...
MAIN = ROOT / "main.py"
SCRIPT = ROOT / "scripts" / "refactor_main_website_lead_existing_patch_core.py"

spec = importlib.util.spec_from_file_location("website_lead_existing_patch_transform", SCRIPT)
transform = importlib.util.module_from_spec(spec)
assert spec and spec.loader
spec.loader.exec_module(transform)


class MainWebsiteLeadExistingPatchTransformTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_legacy_http_and_transport_semantics_are_preserved(self):
        new = transform.NEW
        self.assertIn('await patch_rows(', new)
        self.assertIn('"contactos"', new)
        self.assertIn('{"id": f"eq.{existente[\'id\']}"}', new)
        self.assertIn('"notas": nuevas_notas[:5000]', new)
        self.assertIn('"updated_at": ahora', new)
    ...
    def test_duplicate_success_response_remains_unchanged(self):
        self.assertIn('return {"ok": True, "duplicado": True}', self.transformed)
