"""Dry-run guard for Facebook audience persistence Core migration."""
...
MAIN = ROOT / "main.py"
SCRIPT = ROOT / "scripts" / "refactor_main_fb_audience_post_core.py"

spec = importlib.util.spec_from_file_location("fb_audience_post_transform", SCRIPT)
transform = importlib.util.module_from_spec(spec)
assert spec and spec.loader
spec.loader.exec_module(transform)


class MainFbAudiencePostTransformTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_exact_status_and_logging_contract_is_preserved(self):
        new = transform.NEW
        self.assertIn('await post_rows(', new)
        self.assertIn('"fb_audiences"', new)
        ...
        self.assertIn('_fb_tabla_falta(e.response)', new)
        self.assertIn('_fb_avisa_migracion("guardar público", e.response)', new)
        self.assertIn('e.response.status_code', new)
        self.assertIn('(e.response.text or "")[:200]', new)
        self.assertIn('except Exception as e:', new)
        self.assertIn('_fb_log.error("Error guardando el público: %s", e)', new)
