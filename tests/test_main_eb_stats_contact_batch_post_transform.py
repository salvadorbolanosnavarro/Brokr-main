"""Dry-run guard for EasyBroker stats new-contact batch POST Core routing."""
...
MAIN = ROOT / "main.py"
SCRIPT = ROOT / "scripts" / "refactor_main_eb_stats_contact_batch_post_core.py"

spec = importlib.util.spec_from_file_location("eb_stats_contact_batch_post_transform", SCRIPT)
transform = importlib.util.module_from_spec(spec)
assert spec and spec.loader
spec.loader.exec_module(transform)


class MainEbStatsContactBatchPostTransformTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_status_counters_ids_and_transport_contract_are_preserved(self):
        new = transform.NEW
        ...
        self.assertIn('creados += len(chunk)', new)
        self.assertIn('ids_creados_ok.update(c["id"] for c in chunk)', new)
        self.assertIn('except httpx.HTTPStatusError:\n                errores += len(chunk)', new)
