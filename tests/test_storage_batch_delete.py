...
class StorageBatchDeleteTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_sends_normalized_prefixes_in_one_request(self):
        ...
        with (
            patch("core.storage.settings.require_supabase_service"),
            patch("core.storage.settings.supabase_url", "https://example.supabase.co"),
            patch("core.storage._service_headers", return_value={"Authorization": "Bearer service"}),
