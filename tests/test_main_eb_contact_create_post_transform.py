"""Dry-run guard for /contactos/importar-eb new-contact POST Core routing."""
...
MAIN = ROOT / "main.py"
SCRIPT = ROOT / "scripts" / "refactor_main_eb_contact_create_post_core.py"

spec = importlib.util.spec_from_file_location("eb_contact_create_post_transform", SCRIPT)
transform = importlib.util.module_from_spec(spec)
assert spec and spec.loader
spec.loader.exec_module(transform)


class MainEbContactCreatePostTransformTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_status_counters_caches_and_transport_contract_are_preserved(self):
        new = transform.NEW
        ...
        self.assertIn('existing_by_tel[m["telefono"]] = nuevo', new)
        self.assertIn('existing_by_email[m["email"]] = nuevo', new)
