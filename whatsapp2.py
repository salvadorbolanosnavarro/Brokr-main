...
@router.post("/citas")
async def wa2_citas_crear(req: CitaReq, request: Request):
    user_id = await _require_user(request)
    fila = req.dict()
    fila["user_id"] = user_id
    created = await sb_post("wa2_citas", fila)
    return {"ok": True, "cita": created[0] if created else fila}
...
async def wa2_cita_ics(cita_id: str, request: Request):
    ...
    cita = rows[0]
    ics = _construir_ics(cita["fecha"], cita["hora"], cita.get("titulo") or "Visita", cita.get("notas") or "")
...
async def _get_o_crea_contacto(user_id: str, numero_id: str, wa_id: str, nombre: str | None) -> dict:
    ...
    created = await sb_post("wa2_contactos", {
        ...
        "nombre": nombre, "created_at": _now(), "updated_at": _now(),
    })
    if created:
        return created[0]
    rows = await sb_get("wa2_contactos", {"numero_id": f"eq.{numero_id}", "wa_id": f"eq.{wa_id}",
...
async def _procesar_en_segundo_plano(item: dict):
    ...
    await enviar_push(user_id, contacto.get("nombre") or "Nuevo mensaje (WhatsApp 2.0)",
                      item["texto"], datos={"tipo": "whatsapp2", "conversation_id": item["conversacion_id"]})
    ...
    if isinstance(accion, dict):
        ...
        if tipo == "enviar_inmuebles":
            ...
            if fecha and hora:
                titulo = f"Visita con {contacto.get('nombre') or item['wa_id']}"
                cita = await sb_post("wa2_citas", {
                    "user_id": user_id, "contacto_id": item["contacto_id"], "numero_id": numero["id"],
                    "titulo": titulo, "fecha": fecha, "hora": hora,
                    "notas": accion.get("inmueble") or "", "estado": "pendiente", "created_at": _now()})
                await sb_patch("wa2_contactos", {"id": f"eq.{item['contacto_id']}"}, {"etapa": "Cita"})
                ics = _construir_ics(fecha, hora, titulo, accion.get("inmueble") or "")
                ...
                await enviar_push(user_id, "Nueva cita agendada",
                                  f"{contacto.get('nombre') or item['wa_id']} — {fecha} {hora}",
                                  datos={"tipo": "whatsapp2", "conversation_id": item["conversacion_id"]})
