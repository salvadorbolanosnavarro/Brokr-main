...
and mounts the prepared factory from routers.contact_file_import. It does not
read files, mutate contacts, or invoke any application endpoint.
"""
...
ROUTER_IMPORT = "from routers.contact_file_import import create_router as create_contact_file_import_router\n"
ROUTER_INCLUDE = '''app.include_router(create_contact_file_import_router(lambda: {
...
def main() -> None:
    ...
    if "create_contact_file_import_router" in source:
    ...
    if updated.count("app.include_router(create_contact_file_import_router(") != 1:
