...
SCHEMA = Path("routers/whatsapp_number_schema.py")
TARGET = "NumeroPatchReq"
IMPORT = "from routers.whatsapp_number_schema import NumeroPatchReq\n"
...
if ast.dump(old, include_attributes=False) != ast.dump(canonical, include_attributes=False):
    raise SystemExit("number schema class differs from whatsapp.py")
if IMPORT.strip() in src:
    raise SystemExit("number schema extraction already applied")
