...
SCHEMA = Path("routers/whatsapp_test_schema.py")
TARGET = "ProbarReq"
IMPORT = "from routers.whatsapp_test_schema import ProbarReq\n"
...
if ast.dump(old, include_attributes=False) != ast.dump(canonical, include_attributes=False):
    raise SystemExit("test schema class differs from whatsapp.py")
if IMPORT.strip() in src:
    raise SystemExit("test schema extraction already applied")
