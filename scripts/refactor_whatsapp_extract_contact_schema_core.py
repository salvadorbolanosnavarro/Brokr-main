...
SCHEMA = Path("routers/whatsapp_contact_schema.py")
TARGET = "NotaReq"
IMPORT = "from routers.whatsapp_contact_schema import NotaReq\n"
...
if ast.dump(old, include_attributes=False) != ast.dump(canonical, include_attributes=False):
    raise SystemExit("contact schema class differs from whatsapp.py")
if IMPORT.strip() in src:
    raise SystemExit("contact schema extraction already applied")
