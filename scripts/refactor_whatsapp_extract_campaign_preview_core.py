...
CANONICAL = ROOT / "routers" / "whatsapp_campaign_preview.py"
TARGET = "wa2_campana_audiencia"
CORE_NAME = "wa2_campana_audiencia_core"
IMPORT_MODULE = "routers.whatsapp_campaign_preview"
...
def main():
    ...
    if shape(legacy) != shape(core):
        raise SystemExit("campaign preview body differs")

    wrapper_text = '''async def wa2_campana_audiencia(req: CampanaAudienciaReq, request: Request):
...
'''
    ...
    if any(isinstance(n, ast.ImportFrom) and n.module == IMPORT_MODULE for n in t2.body):
        raise SystemExit("campaign preview already imported")
    ...
    expected = {"_numero_visible", "_audiencia_campana", "WA2_CAMPANA_TOPE"}
    if len(calls) != 1 or {k.arg for k in calls[0].keywords} != expected:
        raise SystemExit("campaign preview wrapper contract differs")
