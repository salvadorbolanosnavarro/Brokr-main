#!/usr/bin/env python3
"""Extract manual read/unread state from the WhatsApp root monolith."""
...
IMPORT_LINE = "from routers.whatsapp_inbox_read_state import router as whatsapp_inbox_read_state_router\n"
ROUTER_ANCHOR = 'router = APIRouter(prefix="/whatsapp2", tags=["whatsapp2"])\n'
INCLUDE_LINE = "router.include_router(whatsapp_inbox_read_state_router)\n"
REMOVE = {"LecturaReq", "wa2_lectura"}
...
def transform_source(source: str) -> str:
    ...
    for forbidden in ("class LecturaReq", "async def wa2_lectura"):
        if forbidden in transformed:
            raise RuntimeError(f"read-state implementation remains: {forbidden}")
    for required in (
        "class ConvPatchReq",
        ...
        if required not in transformed:
            raise RuntimeError(f"adjacent inbox behavior moved unexpectedly: {required}")
