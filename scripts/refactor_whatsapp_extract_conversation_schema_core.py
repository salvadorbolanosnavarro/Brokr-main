...
SCHEMA = Path("routers/whatsapp_conversation_schema.py")
TARGET = "ConvPatchReq"
IMPORT = "from routers.whatsapp_conversation_schema import ConvPatchReq\n"
...
if ast.dump(old, include_attributes=False) != ast.dump(canonical, include_attributes=False):
    raise SystemExit("conversation schema class differs from whatsapp.py")
if IMPORT.strip() in src:
    raise SystemExit("conversation schema extraction already applied")
