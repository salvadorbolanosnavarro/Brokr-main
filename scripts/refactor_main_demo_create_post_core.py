#!/usr/bin/env python3
"""Route only demo scheduling persistence through core.database."""
...
def transform_source(source: str) -> str:
    marker = 'raise HTTPException(status_code=502, detail="No se pudo agendar. Intenta de nuevo en un momento.")'
    if source.count(marker) != 1:
        raise RuntimeError(f"Expected one demo-create marker, found {source.count(marker)}")
    ...
    if old_count == 0 and new_count == 1:
        ...
        return source
    raise RuntimeError(f"Unexpected demo-create POST state: old={old_count}, new={new_count}")
