#!/usr/bin/env python3
"""Extract approved-template sending from the WhatsApp root monolith."""
...
IMPORT_LINE = "from routers.whatsapp_template_send import router as whatsapp_template_send_router\n"
ROUTER_ANCHOR = 'router = APIRouter(prefix="/whatsapp2", tags=["whatsapp2"])\n'
INCLUDE_LINE = "router.include_router(whatsapp_template_send_router)\n"
REMOVE = {"PlantillaEnviarReq", "wa2_enviar_plantilla"}
...
def transform_source(source: str) -> str:
    ...
    for forbidden in ("class PlantillaEnviarReq", "async def wa2_enviar_plantilla"):
        if forbidden in transformed:
            raise RuntimeError(f"template-send implementation remains: {forbidden}")
    if "whatsapp_template_send_router" not in transformed:
        raise RuntimeError("template-send router mount missing")
