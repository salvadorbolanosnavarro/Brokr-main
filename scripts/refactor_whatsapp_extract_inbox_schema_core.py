...
SCHEMA = Path("routers/whatsapp_inbox_schema.py")
TARGET = "EnviarManualReq"
IMPORT = "from routers.whatsapp_inbox_schema import EnviarManualReq\n"
...
if ast.dump(old, include_attributes=False) != ast.dump(canonical, include_attributes=False):
    raise SystemExit("inbox schema class differs from whatsapp.py")
if IMPORT.strip() in src:
    raise SystemExit("inbox schema extraction already applied")
