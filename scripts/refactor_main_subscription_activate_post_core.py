#!/usr/bin/env python3
"""Route only /subscription/activate subscription POST through core.database."""
...
def transform_source(source: str) -> str:
    marker = '@app.post("/subscription/activate")'
    if source.count(marker) != 1:
        raise RuntimeError(f"Expected one subscription activate endpoint, found {source.count(marker)}")
    ...
    if old_count == 0 and new_count == 1:
        ...
        return source
    raise RuntimeError(f"Unexpected activate POST state: old={old_count}, new={new_count}")
