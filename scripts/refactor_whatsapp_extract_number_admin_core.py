...
# Security invariant: the token exposure guard intentionally inspects this
# handler's source and requires the redaction to remain local to the route.
list_node = find_fn(src_tree, "wa2_numeros_list")
list_source = ast.get_source_segment(src, list_node) or ""
if 'r.pop("access_token", None)' not in list_source:
    raise SystemExit("number list token redaction guard missing")
