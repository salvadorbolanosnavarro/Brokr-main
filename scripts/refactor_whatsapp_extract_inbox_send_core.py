#!/usr/bin/env python3
"""Extract manual WhatsApp inbox sending after shared transport/handoff services."""
...
IMPORT_LINE = "from routers.whatsapp_inbox_send import router as whatsapp_inbox_send_router\n"
ROUTER_ANCHOR = 'router = APIRouter(prefix="/whatsapp2", tags=["whatsapp2"])\n'
INCLUDE_LINE = "router.include_router(whatsapp_inbox_send_router)\n"
REMOVE = {"EnviarManualReq", "wa2_enviar_manual"}
...
def transform_source(source: str) -> str:
    ...
    for forbidden in ("class EnviarManualReq", "async def wa2_enviar_manual"):
        if forbidden in transformed:
            raise RuntimeError(f"manual inbox send implementation remains: {forbidden}")
    ...
    for required in (
        ...
        if required not in transformed:
            raise RuntimeError(f"adjacent inbox/template behavior moved unexpectedly: {required}")
