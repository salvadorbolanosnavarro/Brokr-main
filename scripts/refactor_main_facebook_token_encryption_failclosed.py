...
NEW_WARNING = (
    '            "TOKEN_ENC_KEY inválida (%s). Las nuevas escrituras de tokens de Meta "\n'
    '            "se rechazarán hasta corregirla. Genera una con: python3 -c \\\"from "\n'
    '            "cryptography.fernet import Fernet; print(Fernet.generate_key().decode())\\\"", _e)'
)
...
def main() -> int:
    ...
    # Preserve legacy plaintext read compatibility exactly.
    decryptors = [
        node for node in tree.body
    ...
    if OLD_WARNING in transformed:
        transformed = transformed.replace(OLD_WARNING, NEW_WARNING, 1)
    elif "Los tokens seguirán en texto plano" in transformed:
        raise RuntimeError("unexpected invalid-key warning shape")
    ...
    if "return valor" not in out_segment:
        raise RuntimeError("empty/already-encrypted compatibility was lost")
    ...
    if "guardan en texto plano" in out_segment or "return valor\n    try:" in out_segment:
        raise RuntimeError("plaintext write fallback remains")

    # Decryptor must remain byte-for-byte unchanged.
    out_decryptors = [
        node for node in out_tree.body
        if isinstance(node, ast.FunctionDef) and node.name == "descifrar_secreto"
    ]
