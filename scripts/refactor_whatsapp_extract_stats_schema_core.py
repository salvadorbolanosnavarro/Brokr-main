...
SCHEMA = Path("routers/whatsapp_stats_schema.py")
TARGET = "_VENTANAS_ESTAD"
IMPORT = "from routers.whatsapp_stats_schema import _VENTANAS_ESTAD\n"
...
if ast.dump(old.value, include_attributes=False) != ast.dump(canonical.value, include_attributes=False):
    raise SystemExit("stats schema differs from whatsapp.py")
if IMPORT.strip() in src:
    raise SystemExit("stats schema extraction already applied")
