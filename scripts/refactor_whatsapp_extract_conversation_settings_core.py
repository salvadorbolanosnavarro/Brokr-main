#!/usr/bin/env python3
"""Extract WhatsApp conversation AI-mode/stage settings."""
from __future__ import annotations

from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
TARGET = ROOT / "whatsapp.py"

IMPORT_ANCHOR = "from core.storage import delete_objects, upload_object\n"
IMPORT_LINE = "from routers.whatsapp_conversation_settings import router as whatsapp_conversation_settings_router\n"
ROUTER_ANCHOR = 'router = APIRouter(prefix="/whatsapp2", tags=["whatsapp2"])\n'
INCLUDE_LINE = "router.include_router(whatsapp_conversation_settings_router)\n"
START = "class ConvPatchReq(BaseModel):\n"
END = "\n\n@router.delete(\"/mensajes/{mensaje_id}\")\n"


def transform_source(source: str) -> str:
    transformed = source
    if IMPORT_LINE not in transformed:
        if IMPORT_ANCHOR not in transformed:
            raise RuntimeError("Core Storage import anchor not found")
        transformed = transformed.replace(IMPORT_ANCHOR, IMPORT_ANCHOR + IMPORT_LINE, 1)
    if INCLUDE_LINE not in transformed:
        if ROUTER_ANCHOR not in transformed:
            raise RuntimeError("WhatsApp root router anchor not found")
        transformed = transformed.replace(ROUTER_ANCHOR, ROUTER_ANCHOR + INCLUDE_LINE, 1)

    if START in transformed:
        i = transformed.find(START)
        j = transformed.find(END, i)
        if j < 0:
            # Compatible when destructive handlers were extracted first.
            j = transformed.find("\n\nclass NotaReq(BaseModel):\n", i)
        if j < 0:
            raise RuntimeError("conversation-settings end anchor not found")
        transformed = transformed[:i] + transformed[j:]
    elif "class ConvPatchReq" in transformed or "async def wa2_conversacion_patch" in transformed:
        raise RuntimeError("unexpected conversation-settings shape")

    if "class ConvPatchReq" in transformed or "async def wa2_conversacion_patch" in transformed:
        raise RuntimeError("conversation settings remain in whatsapp.py")
    for required in ("async def wa2_enviar_manual", "async def wa2_lectura"):
        if required not in transformed:
            raise RuntimeError(f"other inbox mutation moved unexpectedly: {required}")

    compile(transformed, str(TARGET), "exec")
    return transformed


def main() -> None:
    TARGET.write_text(transform_source(TARGET.read_text(encoding="utf-8")), encoding="utf-8")


if __name__ == "__main__":
    main()
