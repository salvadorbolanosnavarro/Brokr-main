...
`;
  ...
  function renderProfileData(data, user) {
    ...
    if (fdot && ftxt) {
      if (data.fb && data.fb.connected) {
        // El token de Facebook dura ~60 días. Si está por vencer se avisa aquí,
        // que es donde el usuario puede reconectar, y no se pinta en verde algo
        // que está a punto de dejar de funcionar.
        const tk = data.fb.token || {};
        const alerta = tk.expirado || tk.por_expirar;
        fdot.className = 'dot ' + (alerta ? 'warn' : 'ok');
        ftxt.textContent = alerta
          ? (tk.mensaje || 'Conexión por expirar — reconecta tu Facebook')
          : ('Conectado — ' + (data.fb.page_name || 'página vinculada'));
        setDot('pdot-fb', alerta ? 'warn' : 'ok');
        if (fbtn) fbtn.textContent = alerta
          ? 'Reconectar Facebook'
          : 'Cambiar página de Facebook';
  ...
  function connectFacebook() {
    ...
    // leads_retrieval: sin él, el webhook de Lead Ads recibe el aviso pero no
    // puede bajar los datos del prospecto (Meta contesta 403 al leer el lead).
    const scope = 'pages_show_list,pages_read_engagement,pages_manage_posts,pages_manage_metadata,ads_management,ads_read,business_management,leads_retrieval';
    if (!FB_APP_ID) {
    ...
    window._fbOAuthHandler = async function(code) {
      ...
      try {
        ...
        const d = await r.json();
        // El callback ahora falla con status HTTP + `detail` cuando no logra un
        // token de larga duración, en vez de devolver 200 con un token de 1 hora
        // que el usuario guardaba sin enterarse.
        if (!r.ok || !d.ok) {
          throw new Error(d.detail || d.error || 'Error al obtener token de Facebook');
        }
        ...
        const r2 = await fetch(API_BASE + '/facebook/save-page', {
          ...
          body: JSON.stringify({
            ...
            page_token: chosenPage.access_token,
            user_token: d.user_token || '',
            // Sin esto el server no sabe cuándo muere el token y no puede avisar.
            token_expires_at: d.token_expires_at || '',
          })
        });
        if (!r2.ok) throw new Error('Error al guardar la conexión');
        const d2 = await r2.json().catch(() => ({}));
        if (d2 && (d2.scopes_faltantes || []).length && toast) {
          toast.textContent = 'Conectado, pero faltan permisos: ' + d2.scopes_faltantes.join(', ') +
                              '. Reconecta y acepta todos para usar anuncios y Lead Ads.';
          toast.className = 'bk-pd-toast err';
