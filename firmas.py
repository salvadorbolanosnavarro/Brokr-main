...
def _mail_layout(titulo: str, cuerpo: str, boton_texto: str = "", boton_url: str = "") -> str:
    ...
    if boton_texto and boton_url:
        boton = (
            ...
            f'style="display:inline-block;background:#081C4E;color:#ffffff;'
            f'text-decoration:none;padding:14px 28px;border-radius:10px;'
            f'font-weight:700;font-size:15px;">{html.escape(boton_texto)}</a>'
            f'</td></tr>')
    return f"""<!DOCTYPE html><html><body style="margin:0;padding:0;background:#F4F6FB;">
<table width="100%" cellpadding="0" cellspacing="0" style="background:#F4F6FB;padding:32px 16px;">
<tr><td align="center">
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:14px;padding:36px 32px;font-family:'Inter',Helvetica,Arial,sans-serif;color:#0B0B0F;">
<tr><td style="font-size:20px;font-weight:700;letter-spacing:-0.02em;padding-bottom:14px;">{html.escape(titulo)}</td></tr>
<tr><td style="font-size:15px;line-height:1.6;color:#2A3142;">{cuerpo}</td></tr>
{boton}
<tr><td style="padding-top:28px;border-top:1px solid #E7EBF4;margin-top:24px;font-size:12px;color:#8A93A9;line-height:1.5;">
...
async def _invitar(doc: dict, firmante: dict, agente: str) -> Tuple[bool, str, str]:
    ...
    if firmante.get("email"):
        cuerpo = (
            ...
            f"<p style='background:#F4F6FB;border-radius:10px;padding:16px;margin:18px 0;'>"
            ...
            f"<p style='font-size:13px;color:#8A93A9;'>No compartas esta liga: es solo tuya.</p>"
        )
...
async def publico_pedir_codigo(request: Request, token: str, canal: str = Form("")):
    ...
    if not usado and firmante.get("email"):
        cuerpo = (
            ...
            f"background:#F4F6FB;border-radius:10px;padding:20px;text-align:center;"
            f"margin:20px 0;color:#0B0B0F;'>{codigo}</p>"
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  body {{ font-family: "Inter", Helvetica, Arial, sans-serif;
         color: #0B0B0F; font-size: 9.5pt; line-height: 1.5; margin: 0; }}
  .hd {{ border-bottom: 2px solid #081C4E; padding-bottom: 10px; margin-bottom: 16px;
         display: flex; justify-content: space-between; align-items: flex-end; }}
  .hd h1 {{ font-size: 16pt; margin: 0; letter-spacing: -0.02em; }}
  .hd .folio {{ font-size: 13pt; font-weight: 700; color: #081C4E; }}
  .hd .sub {{ font-size: 8.5pt; color: #57607A; margin-top: 3px; }}
  h2 {{ font-size: 10pt; text-transform: none; margin: 20px 0 8px;
        padding-bottom: 5px; border-bottom: 1px solid #DBE1EE; }}
  table {{ width: 100%; border-collapse: collapse; }}
  .resumen td {{ padding: 5px 0; vertical-align: top; border-bottom: 1px solid #E7EBF4; }}
  .resumen .k, .datos .k {{ width: 38%; color: #57607A; }}
  ...
  .firmante {{ border: 1px solid #DBE1EE; border-radius: 6px; padding: 12px 14px;
               margin-bottom: 12px; page-break-inside: avoid; }}
  .firmante__hd {{ display: flex; gap: 10px; align-items: center;
                   border-bottom: 1px solid #E7EBF4; padding-bottom: 8px; margin-bottom: 10px; }}
  .num {{ width: 22px; height: 22px; border-radius: 50%; background: #081C4E; color: #fff;
          font-size: 9pt; font-weight: 700; display: flex; align-items: center;
          justify-content: center; flex: none; }}
  .nom {{ font-size: 11pt; font-weight: 700; }}
  .rol {{ font-size: 8.5pt; color: #57607A; }}
  ...
  .trazo {{ width: 100%; height: 68px; object-fit: contain;
            border: 1px solid #DBE1EE; border-radius: 4px; background: #fff; }}
  .trazo--vacio {{ display: flex; align-items: center; justify-content: center;
                   font-size: 8pt; color: #8A93A9; }}
  .trazo-pie {{ font-size: 7pt; color: #8A93A9; text-align: center; margin-top: 4px; }}
  .datos {{ flex: 1; font-size: 8.5pt; }}
  .datos td {{ padding: 3px 0; border-bottom: 1px solid #F4F6FB; vertical-align: top; }}
  .bitacora {{ font-size: 7.5pt; }}
  .bitacora th {{ text-align: left; color: #57607A; font-weight: 600;
                  border-bottom: 1px solid #DBE1EE; padding: 4px 6px 4px 0; }}
  .bitacora td {{ padding: 3px 6px 3px 0; border-bottom: 1px solid #F4F6FB; vertical-align: top; }}
  .bitacora .ts {{ width: 27%; white-space: nowrap; }}
  .bitacora .ac {{ width: 12%; }}
  .bitacora .ip {{ width: 15%; }}
  .nota {{ background: #F4F6FB; border-radius: 6px; padding: 11px 13px;
           font-size: 8pt; line-height: 1.55; color: #2A3142; margin-top: 8px; }}
  .pie {{ margin-top: 18px; padding-top: 10px; border-top: 1px solid #DBE1EE;
          font-size: 7.5pt; color: #8A93A9; text-align: center; line-height: 1.6; }}
</style></head><body>
...
async def _sellar(documento_id: str) -> None:
    ...
    pie = (
        '<div style="width:100%;font-size:7pt;color:#8A93A9;padding:0 14mm;'
