...
`;
  ...
  let _pdOpen = false;
  let _pdOpenLoadTimer = null;

  function afterNextPaint(fn) {
    requestAnimationFrame(() => requestAnimationFrame(fn));
  }

  function wireProfileDrawerTriggers() {
    const openFast = (ev) => {
      // En móvil, pointerdown se dispara antes que click. Esto hace que el
      // drawer responda al tocar el avatar en vez de esperar el click sintético.
      if (ev.pointerType === 'mouse' && ev.button !== 0) return;
      ev.preventDefault();
      openProfileDrawer();
    };
    ['bk-sb-avatar', 'bk-mob-avatar'].forEach(id => {
      const el = document.getElementById(id);
      if (!el || el._pdFastOpenBound) return;
      el._pdFastOpenBound = true;
      el.addEventListener('pointerdown', openFast, { passive: false });
    });
  }
  ...
  function openProfileDrawer() {
    ...
    // Pintar snapshot/caché sin red antes de abrir: el usuario ve contenido útil
    // en el primer frame y no un panel que espera respuestas externas.
    ...
    // Iniciar la red justo después del primer paint del drawer. Antes había un
    // retraso fijo de 320ms; eso hacía que el perfil se sintiera lento incluso
    // cuando la conexión estaba rápida o había caché.
    if (_pdOpenLoadTimer) window.clearTimeout(_pdOpenLoadTimer);
    afterNextPaint(() => {
      _pdOpenLoadTimer = window.setTimeout(() => {
        _pdOpenLoadTimer = null;
        if (_pdOpen) loadProfileData();
      }, 0);
    });
  }
  ...
  function closeProfileDrawer() {
    _pdOpen = false;
    if (_pdOpenLoadTimer) { window.clearTimeout(_pdOpenLoadTimer); _pdOpenLoadTimer = null; }
